            ...
            case state is
                when IDLE =>
                    if ciphertext_valid = '1' then
                        input_buf(127 downto 96) <= ciphertext_in; state <= LOAD0;
                    end if;
                when LOAD0 =>
                    if ciphertext_valid = '1' then
                        input_buf(95 downto 64) <= ciphertext_in; state <= LOAD1;
                    end if;
                when LOAD1 =>
                    if ciphertext_valid = '1' then
                        input_buf(63 downto 32) <= ciphertext_in; state <= LOAD2;
                    end if;
                when LOAD2 =>
                    if ciphertext_valid = '1' then
                        input_buf(31 downto 0) <= ciphertext_in; state <= LOAD3;
                    end if;
                when LOAD3 =>
                    ...
                    sum <= SUM_START; round_counter <= 0; state <= EXEC_A;
                when EXEC_A =>
                ...
                when EXEC_B =>
                    ...
                    if round_counter = 31 then state <= OUT0;
                    else round_counter <= round_counter + 1; state <= EXEC_A;
                    end if;
                when OUT0 => data_out <= std_logic_vector(v0); valid_flag <= '1'; state <= OUT1;
                when OUT1 => data_out <= std_logic_vector(v1); valid_flag <= '1'; state <= OUT2;
                when OUT2 => data_out <= std_logic_vector(v2); valid_flag <= '1'; state <= OUT3;
                when OUT3 => data_out <= std_logic_vector(v3); valid_flag <= '1'; state <= IDLE;
